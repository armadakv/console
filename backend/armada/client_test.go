...
func setupTest(t *testing.T) (*Client, func()) {
	...
	go func() {
		if err := s.Serve(lis); err != nil {
			t.Logf("Failed to serve: %v", err) //nolint:govet // t.Log in goroutine is safe
		}
	}()
...
func TestGetStatus(t *testing.T) {
	...
	require.NoError(t, err, "GetStatus should not return an error")
...
func TestGetStatusWithServerAddress(t *testing.T) {
	...
	// Check for errors and response using testify/assert.
	require.NoError(t, err, "GetStatus with server address should not return an error")
	assert.Equal(t, "ok", status.Status, "Status should be 'ok'")
	assert.Equal(t, "v1.0.0 — Mock Armada Server", status.Message,
		"Message should be 'v1.0.0 — Mock Armada Server'")
}
...
func TestGetClusterInfo(t *testing.T) {
	...
	// Check for errors and response using testify/assert.
	require.NoError(t, err, "GetClusterInfo should not return an error")
...
func TestGetTables(t *testing.T) {
	...
	// Check for errors and response using testify/assert.
	require.NoError(t, err, "GetTables should not return an error")
...
func TestGetKeyValuePairs(t *testing.T) {
	...
	// Call the method.
	ctx := context.Background()
	result, err := client.GetKeyValuePairs(ctx, "test_table", "key", "", "", "", 10)

	// Check for errors and response using testify/assert.
	require.NoError(t, err, "GetKeyValuePairs should not return an error")
	assert.Len(t, result.Pairs, 2, "Should return 2 pairs")

	assert.Equal(t, "key1", result.Pairs[0].Key, "First key should be 'key1'")
	assert.Equal(t, "value1", result.Pairs[0].Value, "First value should be 'value1'")
	assert.Equal(t, "key2", result.Pairs[1].Key, "Second key should be 'key2'")
	assert.Equal(t, "value2", result.Pairs[1].Value, "Second value should be 'value2'")
}
...
func TestGetKeyValue(t *testing.T) {
	...
	// Check for errors and response using testify/assert.
	require.NoError(t, err, "GetKeyValue should not return an error")
...
func TestPutKeyValue(t *testing.T) {
	...
	require.NoError(t, err, "PutKeyValue should not return an error")
}
...
func TestDeleteKey(t *testing.T) {
	...
	// Check for errors using testify/assert.
	require.NoError(t, err, "DeleteKey should not return an error")
}
...
func TestDeleteTable(t *testing.T) {
	...
	// Check for errors using testify/assert.
	require.NoError(t, err, "DeleteTable should not return an error")
}

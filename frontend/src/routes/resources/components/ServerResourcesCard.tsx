    ...
    <CardWithHeader title="Server Resources">
      {servers.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-slate-400">No server information available</p>
        </div>
      ) : (
          ...
          {servers.map((server) => (
            <div
              ...
              className={`border rounded-lg overflow-hidden border-l-4 ${
                server.isCurrent ? 'border-l-blue-500' : 'border-l-gray-600'
              } border-slate-700`}
            >
              <button
                onClick={() => handleToggle(server.id)}
                className="w-full px-4 py-3 bg-slate-800 border-b border-slate-700 flex items-center justify-between hover:bg-slate-700 transition-colors"
                ...
                <div className="flex items-center space-x-3 flex-1">
                  <div className="flex-1 text-left">
                    <h3 className="font-medium text-white">
                      ...
                      {server.isCurrent && (
                        <span className="text-sm text-slate-400 ml-2">(current)</span>
                      )}
                    </h3>
                    {server.address && (
                      <p className="text-sm text-slate-400 mt-1">({server.address})</p>
                    )}
                  </div>
              ...
              {expanded === server.id && (
                <div id={`${server.id}-content`} className="p-6 bg-slate-900">
